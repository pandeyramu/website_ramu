...
function updateTimeTakenField() {
    ...
    const elapsed = Math.max(0, QUIZ_DURATION_SECONDS - currentTimeLeft);
...
function setupBackNavigationHandling() {
    ...
    window.addEventListener('popstate', () => {
        const shouldLeave = window.confirm('Are you sure you want to leave this quiz? Your attempt in progress will be lost.');
        if (shouldLeave) {
            window.location.replace(exitUrl);
            return;
        }
...
function setupSubmitReviewActions() {
    ...
    submitNowBtn?.addEventListener('click', (event) => {
        event.preventDefault();
        if (!window.confirm('Submit this quiz now? You will not be able to change your answers after submission.')) {
            return;
        }
        allowDirectSubmit = true;
        closeReviewModal();
